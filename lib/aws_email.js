...
const getParametersForAlarm = (alarm, sender, recipient) => {
  const textMessage = alarm;
  const htmlMessage = `<html><pre>${alarm}</pre></html>`;
  const parameters = {
    Destination: { ToAddresses: [recipient] },
    Message: {
      Body: {
      ...
      Subject: { Data: 'Upcoming Event' }
    },
    Source: `Reminder Bot <${sender}>`
  };
  return parameters
}
...
module.exports = {
  getSESEmailParameters,
  getParametersForAlarm,
  sendSESEmail
};
